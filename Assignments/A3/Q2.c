...
// Description: using structures to manage employee information in four departments of Pioneers Limited corporation.
// The program initializes employee attributes such as name, role, communication, teamwork, and creativity randomly. 
// Subsequently, it calculates the sum of values for each employee in each department and determines the "Best Department" 
// based on these sums, presenting the results in a tabular format along with individual employee details.
...
int main()
{
	...
	for (int i = 0; i < 4; i++) {
		if (i == k) continue;
		printf("Employees of department: %s\n\n", departmentNames[i]);
		printEmployees(departments[i]);
		printf("\n");
	}

	printf("Best department award: %s\n\n", departmentNames[k]);
