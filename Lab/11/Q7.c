...
void display_books(Book books[100], int size)
{
	...
	for (int i = 0; i < size; i++) {
		printf("\nBook number: %d\nBook author: %s\nBook  title: %s\nBook issued: %s\n",
			...
			books[i].author,
			books[i].name,
			(books[i].issued) ? "YES" : "NO" // "YES" if books[i].issued is true, and "NO" if it's false.
		);
	}
	printf("\n");
} // end display_books()
...
int add_book(Book books[100], int *size)
{
    ...
    book.author[strlen(book.author) - 1] = '\0'; // remove the newline character  

    printf("Enter book name: ");
    fgets(book.name, sizeof(book.name), stdin);
    book.name[strlen(book.name) - 1] = '\0';    // remove the newline character 
...
int title_count(Book books[100], int size, char *title)
{
	int count = 0;
	for (int i = 0; i < size; i++) {
		if (strcmp(books[i].name, title) == 0) count++;
	    return count;
    } 
} // end title_count()
...
int books_count(Book books[100], int size)
{
	int count = 0;
	for (int i = 0; i < size; i++) {
		if (books[i].issued == false) count++; // if not issued
	}
	return count;
} // end books_count
...
int main()
{
    // array to store the info of all books
    // Book -> Struct      books[100] -> array
	Book books[100] = {
